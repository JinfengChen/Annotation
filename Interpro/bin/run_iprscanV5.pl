...
my $iprscan = "$config{iprscan} -iprlookup -goterms -T . -f TSV";
...
##run the shell file
my $opt;
$opt=" --quene $Queue " if defined $Queue;
`perl $qsub_sge $opt --maxjob $Cpu  --resource nodes=1:ppn=1,mem=8G,walltime=100:00:00 $iprscan_shell_file` if ($Run eq "qsub");
